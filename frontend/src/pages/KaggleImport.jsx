...
const KaggleImport = () => {
  ...
  const hasLoadedActiveWorkspace = useMemo(() => (
    Boolean(activeProjectId) && Object.prototype.hasOwnProperty.call(allDatasets, activeProjectId)
  ), [activeProjectId, allDatasets]);
  ...
  const handleFilesUploaded = useCallback(async (files) => {
    ...
    setUploadDone(true);
    setImportStatus('Files uploaded successfully.');
    void reloadWorkspaceData(workspaceId).catch((err) => {
      console.error('Failed to refresh workspace after upload:', err);
    });
        ...
        <section className="space-y-6">
          ...
          <div className="glass-panel p-5 sm:p-6 lg:p-8 rounded-xl inner-glow">
            <FileUploadUploadThingDemo
              ...
              onReset={() => { setUploadDone(false); setImportStatus(''); }}
            />
          </div>
        </section>
