...
const TopHeader = ({ title, subtitle, showSearch = false, onSearchChange, actionButton }) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('left-sidebar-state-changed', handleStateChange);
    window.dispatchEvent(new CustomEvent('request-left-sidebar-state'));
    return () => {
      window.removeEventListener('left-sidebar-state-changed', handleStateChange);
    };
  }, []);
    ...
    <header className="app-top-header sticky top-0 z-40 flex min-h-[72px] w-full items-center border-b border-outline-variant/30 bg-surface/85 px-4 shadow-sm shadow-slate-900/5 backdrop-blur-md sm:px-6 lg:px-12">
      <div className="flex w-full min-w-0 items-center gap-3 sm:gap-4 lg:gap-6">
        <button
          ...
          className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-[#2a2a2d] bg-[#18181b] text-[#dcdce2] shadow-lg shadow-black/30 transition-all hover:bg-[#232327] hover:text-white md:hidden cursor-pointer"
