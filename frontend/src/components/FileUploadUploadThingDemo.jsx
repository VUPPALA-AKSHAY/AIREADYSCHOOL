...
function UploadNowButton({ fileCount, isUploading, isProcessing }) {
  ...
  return (
    <button
      ...
      `}
    >
      {isUploading ? "Uploading..." : isProcessing ? "Processing..." : `Upload ${fileCount} ${fileCount === 1 ? "file" : "files"}`}
    </button>
  );
}
...
export function FileUploadUploadThingDemo({ onFilesUploaded, disabled = false, uploadDone = false, onReset }) {
            ...
            <div className="mt-2 space-y-1">
              {res.map((f) => (
                <div key={f.name} className="text-xs font-mono" style={{ color: "#4ade80" }}>
                  {`Uploaded: ${f.name.length > 32 ? `${f.name.slice(0, 32)}...` : f.name}`}
                </div>
              ))}
            </div>
          ),
  ...
  return (
    <FileUpload
      ...
      multiple
      disabled={isBusy || disabled || uploadDone}
    >
      <FileUploadDropzone>
      ...
      {files.length > 0 && !uploadDone && (
        <div className="mt-4 flex items-center justify-between px-1">
          ...
          <UploadNowButton fileCount={files.length} isUploading={isUploading} isProcessing={isProcessing} />
        </div>
      )}
