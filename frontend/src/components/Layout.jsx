...
const Layout = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const handleRequest = () => {
      window.dispatchEvent(new CustomEvent('left-sidebar-state-changed', { detail: { open: sidebarOpenRef.current } }));
    };
    window.addEventListener('toggle-left-sidebar', handleToggle);
    window.addEventListener('request-left-sidebar-state', handleRequest);
    return () => {
      window.removeEventListener('toggle-left-sidebar', handleToggle);
      window.removeEventListener('request-left-sidebar-state', handleRequest);
    };
  }, []);

  useEffect(() => {
    window.dispatchEvent(new CustomEvent('left-sidebar-state-changed', { detail: { open: sidebarOpen } }));
  }, [sidebarOpen]);
  ...
  return (
    <div className="flex h-[100dvh] overflow-hidden bg-[#0f0f10] text-on-surface">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex h-[100dvh] items-center justify-center bg-[#0f0f10] transition-all duration-300 ease-out ${
        sidebarOpen ? 'w-[min(340px,calc(100vw-12px))] lg:w-[340px] translate-x-0' : 'w-[76px] lg:w-[92px] -translate-x-full md:translate-x-0'
      }`}>
      ...
      <div className={`relative flex h-[100dvh] min-w-0 flex-1 flex-col bg-surface-container-lowest transition-[margin] duration-300 ease-out pb-16 md:pb-0 ml-0 md:${
          ...
          return (
            <NavLink
              ...
              className={`flex flex-col items-center justify-center flex-1 py-1 text-center transition-colors ${
                isActive ? 'text-white' : 'text-[#8f8f98] hover:text-white'
              }`}
            >
