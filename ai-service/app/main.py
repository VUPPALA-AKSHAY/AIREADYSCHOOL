...
def _build_prompt(
    ...
    if hits:
        source_context = "\n\n".join(
            (
                ...
                f"{_strip_internal_metadata(hit.chunk.text)}"
            )
            for hit in hits
        )
    else:
    ...
    supplied_context = _truncate_text(_strip_internal_metadata(supplied_context or ""), SUPPLIED_CONTEXT_CHARS)
...
def _build_direct_document_prompt(question: str, supplied_context: str | None, has_images: bool = False) -> str:
    selected_document_text = _truncate_text(_strip_internal_metadata(supplied_context or ""), DIRECT_CONTEXT_CHARS)
...
async def _generate_with_gemini(
    *,
    ...
    for attempt_key_idx, current_key in enumerate(keys_to_try):
        url = GEMINI_API_URL.format(model=_model_path_name(model))
        masked_key = f"{current_key[:8]}...{current_key[-6:]}" if current_key and len(current_key) > 14 else "None"
        print(f"[Gemini Call] model={model} key={masked_key} url={url} has_images={bool(images)} (key source: {'primary' if attempt_key_idx == 0 else 'fallback'})")
        ...
        try:
            async with httpx.AsyncClient(timeout=45, trust_env=False) as client:
                response = await client.post(f"{url}?key={current_key}", json=payload)
        except HTTPError as error:
            if attempt_key_idx < len(keys_to_try) - 1:
                print(f"Gemini connection failed for key {masked_key}: {error}. Trying fallback key...")
                continue
            raise HTTPException(
                status_code=502,
                detail={
        ...
        if response.status_code >= 400 and model != "gemini-2.5-flash":
            ...
            try:
                async with httpx.AsyncClient(timeout=45, trust_env=False) as client:
                    response = await client.post(f"{fallback_url}?key={current_key}", json=payload)
            except Exception as err:
        ...
        if attempt_key_idx < len(keys_to_try) - 1:
            print(f"Gemini API request failed for key {masked_key} with status {response.status_code}: {response.text[:200]}. Trying fallback key...")
            continue

        # If the last key also failed, raise exception
        raise HTTPException(
            status_code=502,
            detail={
...
# Removes internal metadata labels injected during ingestion that should never
# appear in AI responses (e.g. "Summary: ...", "Insights: ...", "Topics: ...").
_INTERNAL_METADATA_RE = re.compile(
    r"^(?:Summary|Insights?|Topics?)\s*:\s*.*$",
    re.IGNORECASE | re.MULTILINE,
)

def _strip_internal_metadata(text: str) -> str:
    """Strip Summary/Insights/Topics label lines from chunk or context text."""
    return _INTERNAL_METADATA_RE.sub("", text).strip()
...
async def _generate_with_groq(
    *,
    ...
    if not text:
        raise HTTPException(status_code=502, detail={"message": "Groq returned an empty response", "model": model})
...
async def _generate_response(
    *,
    ...
    except HTTPException as frenix_error:
        # --- Fallback 1: Groq (openai/gpt-oss-120b) ---
        groq_key = os.getenv("GROQ_API_KEY")
        if groq_key:
            try:
                print(f"Frenix failed ({frenix_error.status_code}). Falling back to Groq ({GROQ_MODEL})…")
                ...
                print(f"Groq fallback also failed ({groq_error.status_code}). Trying Gemini…")

        # --- Fallback 2: Gemini ---
        can_fallback_to_gemini = bool(gemini_api_key)
        if frenix_error.status_code < 500 or not can_fallback_to_gemini:
            raise frenix_error
